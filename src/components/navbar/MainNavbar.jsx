    ...
    ({ icon, title, description, link }, key) => (
      <a href={link} key={key}>
        <MenuItem className="flex items-center gap-3 rounded-lg">
          <div className="flex items-center justify-center rounded-lg  hover:text-[#da0404] p-2 ">
            {" "}
      ...
      >
        <MenuHandler>
          <Typography as="div" variant="small" className="font-medium">
            <ListItem
              className="flex items-center gap-2 py-2 pr-4 font-space text-xs text-[#da0404] font-thin hover:bg-[#1f2937] hover:text-[#da0404]"
              selected={isMenuOpen || isMobileMenuOpen}
              onClick={() => setIsMobileMenuOpen((cur) => !cur)}
            >
              <Square3Stack3DIcon className="h-[18px] w-[18px] text-[#da0404]" />{" "}
              ~cd Support
              <ChevronDownIcon
                strokeWidth={2.5}
...
function NavListMenu() {
  ...
  return (
    <React.Fragment>
      <Menu allowHover open={isMenuOpen} handler={setIsMenuOpen}>
        <MenuHandler>
          <Typography
            ...
            href="#"
            variant="small"
            className="font-normal text-[#da0404] "
          >
            <MenuItem className="hidden items-center gap-2 font-light font-space text-xs  text-[#da0404]  lg:flex lg:rounded-full hover:bg-[#1f2937] hover:text-[#da0404] ">
              <Square3Stack3DIcon className="h-[18px] w-[18px] text-[#da0404]" />{" "}
              ~cd community{" "}
              <ChevronDownIcon
                strokeWidth={2}
                className={`h-3 w-3 transition-transform text-[#da0404] ${
                  isMenuOpen ? "rotate-180" : ""
                }`}
              />
            </MenuItem>
      ...
      <MenuItem className="flex items-center gap-2 font-light font-space text-xs  text-[#da0404] lg:hidden  hover:bg-[#1f2937] hover:text-[#da0404]">
        <Square3Stack3DIcon className="h-[18px] w-[18px] text-[#da0404]" /> ~cd
        community{" "}
      </MenuItem>
    ...
    <ul className="mt-2 mb-4 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center ">
      {navListItems.map(({ label, icon, link }, key) => (
        ...
        <Typography
          ...
          variant="small"
          color="gray"
          className="font-light font-space text-xs  text-[#da0404]"
        >
          <MenuItem className="flex items-center gap-2 lg:rounded-full hover:bg-[#1f2937]">
            {React.createElement(icon, {
              className: "h-[18px] w-[18px] text-[#da0404]",
            })}{" "}
            <span className="text-[#da0404] "> {label}</span>
          </MenuItem>
        </Typography>
        // </Link>
      ))}
...
export function MainNavbar() {
  ...
  return (
    ...
    >
      <div className="relative mx-auto flex items-center justify-between text-[#da0404]">
        <Typography
          as="a"
          href="#"
        ...
        {/* <Button size="sm" variant="text" className="hover:bg-[#1f2937] text-[#da0404] font-space text-xs">
          <span>Log In</span>
        </Button> */}
        <IconButton
          size="sm"
